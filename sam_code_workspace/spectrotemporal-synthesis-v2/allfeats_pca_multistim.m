function [pca_timecourse_MAT_files, pca_weight_MAT_files, model_features, ...
    pca_timecourses_allstim_allmodels, coch_output_directory, ...
    modulation_output_directory, pca_output_directories] = allfeats_pca_multistim(...
...
% whether or not to save highres features before downsampling
if ~isfield(P_orig, 'save_highres')
    P_orig.save_highres = false;
end
        ...
        if ~exist(filtcoch_MAT_files{j}, 'file') || P_orig.overwrite
            ...
            switch P_orig.nonlin
                ...
                case 'rect'
                    ...
                    filtcoch = coch2filtcoch_allsubbands(...
                        padded_coch, P_mod, complex_filters, fourier_domain, separable);
                    filtcoch = cat(5, max(filtcoch, 0), max(-filtcoch, 0));
                otherwise
                    error('No matching case');
            end
...
for i = 1:n_model_features
    ...
    for j = 1:length(stimuli)
        ...
        if ~exist(pca_timecourse_MAT_files{j,i}, 'file') || P_orig.overwrite
            ...
            % save results
            F = pca_timecourses;
            save(pca_timecourse_MAT_files{j,i}, 'F', 'P', 'dims', '-v7.3');
            clear F;

        elseif P_orig.return_PCA_timecourses
            
            load(pca_timecourse_MAT_files{j,i}, 'F', 'P', 'dims');
